...
def convert_m4a_to_mp4(m4a_file, output_mp4=None):
    """Convert M4A file to MP4 with a static image."""
    if not os.path.exists(m4a_file):
        print(f"Error: {m4a_file} does not exist.")
        return None
    ...
    # Use FFmpeg to convert m4a to mp4
    try:
        cmd = [
            "ffmpeg", "-y", "-loop", "1", "-i", image_path,
            "-i", m4a_file, "-c:v", "libx264", "-tune", "stillimage",
            "-c:a", "aac", "-b:a", "192k", "-pix_fmt", "yuv420p",
            "-shortest", output_mp4
        ]
        subprocess.run(cmd, check=True)
        print(f"Successfully converted {m4a_file} to {output_mp4}")
        return output_mp4
    except subprocess.CalledProcessError as e:
        print(f"Error converting M4A to MP4: {e}")
        return None
...
def main():
    ...
    if len(sys.argv) < 2 or sys.argv[1] == "--help":
        print("Usage: python m4a_to_text.py <m4a_file_path> [language_code] [output_filename]")
        print("\nSupported language codes:")
        for code, name in supported_languages.items():
            print(f"  {code} - {name}")
        print("\nExamples:")
        print("  python m4a_to_text.py recording.m4a zh")
        print("  python m4a_to_text.py recording.m4a zh my_transcript")
        return
